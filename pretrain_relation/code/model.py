...
from transformers import BertForMaskedLM, BertForPreTraining, BertTokenizer, BertModel
...
class CP(nn.Module):
    ...
    def forward(self, input, mask, label, h_pos, t_pos, rinput, rmask, rtype, rlabel):
        ...
        h_state = outputs.hidden_states[-1][indice, h_pos] # (batch_size * 2, hidden_size)
        t_state = outputs.hidden_states[-1][indice, t_pos]
        state = torch.cat((h_state, t_state), 1)

        r_loss = self.ntxloss(state, label)

        sample_cls = outputs.hidden_states[-1][:, 0, :]
        rcls = self.rbert(rinput, rmask, rtype).pooler_output

        srlogits = (sample_cls.reshape(batch_size * 2, 1, -1) * rcls.reshape(1, batch_size, -1)).sum(dim=-1)
        srlabel = torch.stack([torch.arange(0, batch_size, device=srlogits.device)] * 2, dim=0).transpose(0, 1).reshape(-1)
        r_loss += nn.CrossEntropyLoss()(srlogits, srlabel)

        # r_loss += self.ntxloss(torch.cat((sample_cls, rcls), dim=0), torch.cat((label, rlabel), dim=0))
