...
def collate_fn(data, tokenizer):
    v1, v2, v3, v4, v5, v6, v7 = list(zip(*data))
    rel = tokenizer(v6, max_length=500, truncation=True, padding=True, return_tensors='pt')
    v1 = torch.tensor(v1)
    v2 = torch.tensor(v2)
    v3 = torch.tensor(v3)
    v4 = torch.tensor(v4)
    v5 = torch.tensor(v5)
    v7 = torch.tensor(v7)
    return v1, v2, v3, v4, v5, v7, rel['input_ids'], rel['attention_mask'], rel['token_type_ids']
...
def train(args, model, train_dataset):
    ...
    train_dataloader = data.DataLoader(train_dataset, collate_fn=functools.partial(collate_fn, tokenizer=BertTokenizer.from_pretrained('../../bert-base-uncased')), **params)
    ...
    for i in range(args.max_epoch):
        ...
        for step, batch in enumerate(train_dataloader):
            if args.model == "MTB":
                ...
                inputs = {"input": batch[0].to(args.device), "mask": batch[1].to(args.device), "label": batch[2].to(args.device),
                          "h_pos": batch[3].to(args.device), 't_pos': batch[4].to(args.device), 'rlabel': batch[5].to(args.device),
                          'rinput': batch[6].to(args.device), 'rmask': batch[7].to(args.device), 'rtype': batch[8].to(args.device)}
            model.train()
            with autocast():
            ...
            if step % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [0, -1] and global_step % args.save_step == 0:
                    ...
                    ckpt = {
                        ...
                        'rbert': model.module.rbert.state_dict()
                    }
...
if __name__ == "__main__":
    ...
    if args.local_rank == -1:
        ...
        devices = args.cuda.split(",")
        d = int(devices[args.local_rank])
