...
class REDataset(data.Dataset):
    ...
    def __init__(self, path, mode, args):
        ...
        with open(os.path.join(path, mode)) as f:
            ...
            for line in all_lines:
                ...
                if 'token' in ins:
                    ...
                    tmp = {'token': [], 'h': {'pos': [0, 0]}, 't': {'pos': [0, 0]}, 'relation': None}
                    t = ins['text'].replace('<<', ' << ').replace('>>', ' >> ').replace('[[', ' [[ ').replace(']]', ' ]] ')
                    for tok in t.split(' '):
                        if len(tok) == 0:
                            continue
                        if tok == '<<':
                            tmp['h']['pos'][0] = len(tmp['token'])
                        elif tok == '>>':
                            tmp['h']['pos'][1] = len(tmp['token'])
                        elif tok == '[[':
                            tmp['t']['pos'][0] = len(tmp['token'])
                        elif tok == ']]':
                            tmp['t']['pos'][1] = len(tmp['token'])
                        else:
                            tmp['token'].append(tok)
                    tmp['relation'] = ins['label']
                    data.append(tmp)
        ...
        for i, ins in enumerate(data):
            self.label[i] = rel2id[ins["relation"] if 'relation' in ins else ins['label']]
            # tokenize
            if args.mode == "CM":
