...
def train(args, model, train_dataloader, dev_dataloader, test_dataloader, devBagTest=None, testBagTest=None):
    ...
    for i in range(args.max_epoch):
        ...
        with torch.no_grad():
            ...
            if args.dataset == "semeval" or args.dataset == "tacred" or args.dataset == "chemprot":
                eval_func = eval_F1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--load_rbert', action='store_true')

    parser.add_argument('--freeze_rbert', action='store_true')
    ...
    model = REModel(args, load_rbert=args.load_rbert)
    model.cuda()
    ...
    rel = tokenizer(keys, padding=True, return_tensors='pt')
    rel.to('cuda')
