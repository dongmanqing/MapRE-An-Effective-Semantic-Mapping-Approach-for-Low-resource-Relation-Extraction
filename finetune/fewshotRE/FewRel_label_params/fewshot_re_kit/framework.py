...
class FewShotREModel(nn.Module):
    ...
    def loss(self, logits, label):
        ...
        '''
        N = logits.size(-1)
        # rs_loss = self.cost(rs_logits.view(-1, N), torch.arange(N, device=rs_logits.device).repeat(rs_logits.size(0)))
        return nn.NLLLoss()(logits.view(-1, N), label.view(-1))
        ...
        for it in range(start_iter, start_iter + train_iter):
            with autocast():
                if pair:
                    ...
                    support, query, label, desc = next(self.train_data_loader)
                    if torch.cuda.is_available():
                        desc = {k: v.cuda() for k, v in desc.items()}
                        for k in support:
                    ...
                    logits, pred  = model(support, query, desc,
        ...
        with torch.no_grad():
            for it in range(eval_iter):
                if pair:
                    ...
                    support, query, label, desc = next(eval_dataset)
                    if torch.cuda.is_available():
                        desc = {k: v.cuda() for k, v in desc.items()}
                        for k in support:
                            support[k] = support[k].cuda()
                        for k in query:
                            query[k] = query[k].cuda()
                        label = label.cuda()
                    logits, pred = model(support, query, desc, N, K, Q * N + Q * na_rate)
                ...
                if pair:
                    ...
                    if torch.cuda.is_available():
                        for k in batch:
                            batch[k] = batch[k].cuda()
                        label = label.cuda()
                    logits, pred = model(batch, N, K, Q + Q * na_rate)
                else:
                    support, query, label, desc = it
                    if torch.cuda.is_available():
                        desc = {k: v.cuda() for k, v in desc.items()}
                        for k in support:
                            support[k] = support[k].cuda()
                        for k in query:
                            query[k] = query[k].cuda()
                        label = label.cuda()
                    logits, pred = model(support, query, desc, N, K, Q + Q * na_rate)
                res += pred.detach().cpu().numpy().tolist()
