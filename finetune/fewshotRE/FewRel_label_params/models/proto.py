...
class Proto(fewshot_re_kit.framework.FewShotREModel):
    ...
    def __init__(self, sentence_encoder, pretrained_path, path, hidden_size=230, alpha=0.5):
        ...
        d = torch.load(path)
        if 'rbert' in d:
            self.bert.load_state_dict(d['rbert'], strict=False)
        self.alpha = nn.Parameter(torch.tensor(0.95, dtype=torch.float, device='cuda'))
        self.beta = nn.Parameter(torch.tensor(1.05, dtype=torch.float, device='cuda'))
    ...
    def forward(self, support, query, desc, N, K, total_Q):
        ...
        p = self.alpha * logits + self.beta * rq_logits
